...
int main()
{
	...
	namedWindow("CThrshld", WINDOW_AUTOSIZE);
	Mat cimg(simg.rows, simg.cols, CV_8UC1, Scalar(0));
	createTrackbar("Low", "CThrshld", &l, 255);
	createTrackbar("High", "CThrshld", &h, 255);

	while (1)
	{
		for (i = 0; i < simg.cols; i++)
		{
			for (j = 0; j < simg.rows; j++)
			{
				if (simg.at <uchar>(j, i)>l && simg.at <uchar>(j, i) < h)
					cimg.at <uchar>(j, i) = simg.at <uchar>(j, i);
				else
					cimg.at<uchar>(j, i) = 0;
			}
		}
		imshow("CThrshld", cimg);
		waitKey(10);
	}
	return 0;
}*/
